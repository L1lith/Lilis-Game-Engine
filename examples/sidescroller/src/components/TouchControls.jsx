...
export default function JumpButton({entity}) {
    ...
    return <div onContextMenu={e => e.preventDefault()} class="touchControls">
        <button ref={movementButton} onMouseDown={movementButtonDown} onMouseMove={movementButtonMoved} onMouseOut={movementButtonUp} onMouseUp={movementButtonUp} class={"move" + (movementButtonHeld() ? ' held' : '')}><img draggable={false} src="left-right.png" alt="left and right movement icon"/></button>
        <button onTouchStart={jumpButtonDown} onTouchEnd={jumpButtonUp} onTouchCancel={jumpButtonUp} onMouseOut={jumpButtonUp} onMouseDown={jumpButtonDown} onMouseUp={jumpButtonUp} class={"jump" + (jumpButtonHeld() ? ' held' : '')}><img draggable={false} src="jump-button.png" alt="jump icon"/></button>
    </div>
}