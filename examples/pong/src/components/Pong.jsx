...
export default function Pong() {
    ...
    onMount(async ()=>{
        ...
        const renderSettings = RenderSettings({canvas, calculateDimensions: (width, height) => ([Math.min(width, height), Math.min(width, height)])})
        renderSettings.p = (p)=>{
            p.resizeCanvas(p.windowWidth, p.windowHeight)
        }
        // Write the main game code
        entities.addChild(Entity({ // Background
            shape: 'rect',
            x: 0,
            y: 0,
            width: 100,
    ...
    return <canvas style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);" ref={canvas}/>
}