...
export default function Pong() {
  ...
  onMount(async () => {
    ...
    const tableSplit = entities.addChild(
      Entity({
        color: "white",
        stroke: infoColor,
        render: (p) => {
          ...
          p.strokeWeight(4);
          p.line(p.width / 2, 0, p.width / 2, p.height * .45);
          p.strokeWeight(0);
        },
        renderPriority: -10,
      }),
    );
    const tableSplit2 = entities.addChild(
      Entity({
        color: "white",
        stroke: infoColor,
        render: (p) => {
          p.drawingContext.setLineDash([5, 10, 30, 10]);
          p.strokeWeight(4);
          p.line(p.width / 2, p.height * .55, p.width / 2, p.height);
          p.strokeWeight(0);
        },
        renderPriority: -10,
      }),
    ...
    const handlePlayerScore = (winner) => {
      ...
      console.log(
        `Player ${winner} scored!, total score: ${targetScore.get()}`,
      );
    };
    ...
    const player2ScoreDisplay = entities.addChild(
      Entity({
        fill: infoColor,
        render: (p, positionData) => {
          const { x, y } = positionData;
    ...
    const ballManager = {
      tick: () => {
        ...
        if (
          Body.getSpeed(ball.matterBody) === 0 ||
          isOffscreenLeft ||
          isOffscreenRight
        ) {
          const xForce = (ball.matterBody.force.x =
            ((ballInitialForce / 2) * Math.random() + ballInitialForce / 2) *
            (Math.random() < 0.5 ? -1 : 1));
          ball.matterBody.force.y =
            Math.min(
              (ballInitialForce / 2) * Math.random(),
              ballInitialForce / 2,
              Math.abs(xForce),
            ) * (Math.random() < 0.5 ? -1 : 1); // Math.min here prevents us from shooting upwards balls which take forever to score.
    ...
    const enforceBounds = (paddleY, entity) =>
      Math.min(
        50 - entity.height / 2,
        Math.max(-50 + entity.height / 2, paddleY),
      );
    const playerController = {
      tick: () => {
        ...
        leftPaddle.y = enforceBounds(
          leftPaddle.y + player1Direction * playerSpeed,
          leftPaddle,
        );
        rightPaddle.y = enforceBounds(
          rightPaddle.y + player2Direction * playerSpeed,
          rightPaddle,
        );
      },
    };
