...
export default function SpikeVSSpace() {
  ...
  onMount(async () => {
    ...
    let castle
    const resetCastle = ()=>{
      if (castle) entities.removeChild(castle)
    ...
    const dragRubberBandsTo = (targetX, targetY, ignoreSpike = false) => {
      ...
      const distance = calculateDistance(
        targetX,
        targetY,
        rubberBandRestingPoint.x,
        rubberBandRestingPoint.y,
      );
      const { x, y } = clampAngleToRange(
        rubberBandRestingPoint,
        getPointAtDistance(
          rubberBandRestingPoint,
          { x: targetX, y: targetY },
          maxRubberBandLength,
        ...
        rubberBandAngleCenter,
        rubberBandDegreesOfFreedom,
      );

      const rubberBandAClampedX = Math.min(x, rubberBandRestingPoint.x - 4);
      if (!ignoreSpike) {
        const { x: spikeX, y: spikeY } = clampAngleToRange(
          rubberBandRestingPoint,
          getPointAtDistance(
            rubberBandRestingPoint,
            { x: targetX, y: targetY },
            Math.min(
              Math.max(distance - spikeInsetDistance, 0),
              maxRubberBandLength - spikeInsetDistance,
            ),
            true,
          ),
          rubberBandAngleCenter,
      ...
      const rubberBandADistance = calculateDistance(
        ignoreSpike ? x : rubberBandAClampedX,
        y,
        rubberBandAttachmentA.x,
        rubberBandAttachmentA.y,
      );
      const rubberBandBDistance = calculateDistance(
        x,
        y,
        rubberBandAttachmentB.x,
        rubberBandAttachmentB.y,
      );
      rubberBandA.width = rubberBandADistance;
      rubberBandB.width = rubberBandBDistance;
      const { x: bandAx, y: bandAy } = calculateCenter(
        rubberBandAttachmentA.x,
        rubberBandAttachmentA.y,
        ignoreSpike ? x : rubberBandAClampedX,
        y,
      );
      const { x: bandBx, y: bandBy } = calculateCenter(
        rubberBandAttachmentB.x,
        rubberBandAttachmentB.y,
        x,
        y,
      ...
      const rubberBandARotation = calculateAngle(
        ignoreSpike ? x : rubberBandAClampedX,
        y,
        rubberBandAttachmentA.x,
        rubberBandAttachmentA.y,
      );
      const rubberBandBRotation = calculateAngle(
        x,
        y,
        rubberBandAttachmentB.x,
        rubberBandAttachmentB.y,
    ...
    const isTouchingSlingshot = (e) => {
      ...
      return (
        target === canvas &&
        worldX < rubberBandRestingPoint.x + 5 &&
        worldY > rubberBandRestingPoint.y - 10
      );
    };
    ...
    const slingshotTouchEndListener = (e) => {
      if (isDraggingSlingshot() && isTouchingSlingshot(e)) {
        ...
        let xFactor =
          (rubberBandRestingPoint.x - spike.x) * force * launchForceModifier;
        let yFactor =
          (rubberBandRestingPoint.y - spike.y) * force * launchForceModifier;
        spike.static = false;
    ...
    const returnToRestPlugin = {
      tick: () => {
        if (!launchPoint() || !isFinite(launchTime()) || launchTime === null)
          return;
        const transitionPercent =
          (Date.now() - launchTime()) / returnToRestTime;
        const startingDistance = calculateDistance(
          launchPoint().x,
          launchPoint().y,
          rubberBandRestingPoint.x,
          rubberBandRestingPoint.y,
        ...
        const currentDistance = Math.max(
          0,
          startingDistance * (1 - transitionPercent),
        );
        const { x, y } = getPointAtDistance(
          rubberBandRestingPoint,
          launchPoint(),
          currentDistance,
          true,
    ...
    const detectLaunchFinishedPlugin = {
      tick: () => {
        if (
          spike.x > 50 + spike.width / 2 ||
          spike.x < -50 - spike.width / 2 ||
          spike.y > 50 + spike.height / 2 ||
          spike.y < -50 - spike.height / 2
        ) {
